...
function updateChart() {
    ...
    fetch('/data_distancia', { signal: signal })
    .then(response => {
        if (!response.ok) {
            //espera 3 segundos
            setTimeout(() => {
    ...
    .then(data => {
        ...
        if (data.distance !== undefined) {
            ...
            myChart.data.datasets[0].data.push({ x: currentTime, y: data.distance }); // Agregar punto
            myChart.data.datasets[1].data.push({ x: currentTime, y: data.distance }); // Agregar punto a la línea
            ...
            console.log(`Data point added: ${currentTime} ms, ${data.distance} cm`);
        } else {
