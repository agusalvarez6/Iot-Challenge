...
class Sensor:
    def __init__(self, baud_rate=115200):
        ...
        self.running = True
        self.thread = threading.Thread(target=self.read_data, name="SensorDistanciaThread")
        ...
        while self.running:
            try:
                if self.ser is not None and self.ser.is_open:
                    self.ser.flushInput()
                    self.ser.write(b'A')  # Enviar comando para solicitar datos A distancia P contacto
                    ...
                    if line:
                        try:
                            distance = float(line)
                            with self.lock:
                                self.latest_distance = {'distance': distance}
                                with open("val", 'w') as archivo:
                                    json.dump(self.latest_distance, archivo)  # Convertir el diccionario a JSON
                                    archivo.write("\n")
                                failed_attempts = 0
                        except ValueError:
